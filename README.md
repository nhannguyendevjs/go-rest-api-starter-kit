...
```bash
docker run --name golang-ubuntu --network local-network -p 80:8080 -p 443:8443 -p 22:22 -itd ubuntu:latest
```

### MongoDB

```bash
docker run -d --network local-network --name golang-mongo -p 27017:27017 -e MONGO_INITDB_ROOT_USERNAME=admin -e MONGO_INITDB_ROOT_PASSWORD=admin mongo:latest

docker exec -it golang-mongo mongosh
```
...
```bash
docker run --name golang-postgres --network local-network -p 5432:5432 -e POSTGRES_DB=golang -e POSTGRES_USER=admin -e POSTGRES_PASSWORD=admin -d postgres:latest

docker exec -it golang-postgres psql -U admin -d golang
```

#### URI

```txt
postgres://admin:admin@localhost:5432/golang?schema=public
```

### Redis

```bash
docker run -d --network local-network --name golang-redis -p 6379:6379 redis:latest
```
